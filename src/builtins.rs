...
///
/// This is the *JavaScript function's parameter count* in jsonata-js
/// (`hofFuncArgs` -> `getFunctionArity` -> `implementation.length`), not the
/// JSONata signature's arity -- `$string` has two signature parameters but
/// arity 1, and `$substring` has arity 3, which is why
/// `$map([1,2], $substring)` is a T0410 in the reference (arity 3 is more
/// than the 1 argument `$map` supplies its callback).
///
...
/// `#[cfg(test)] arity_table_matches_fixture` below pins this table against
/// that fixture -- a submodule bump that changes an arity must fail CI, not
/// drift silently.
///
/// Returns `None` for a name this table does not cover; the caller should
/// treat that as "unknown, do not truncate" (`usize::MAX`), not as arity 0 --
/// `zip` and `millis` are legitimately arity 0 and are returned as
/// `Some(0)`.
...
mod tests {
    ...
    fn arity_table_matches_fixture() {
        ...
        assert_eq!(
            ...
            fixture
                .keys()
                .filter(|name| builtin_arity(name).is_some())
                .count(),
            "builtin_arity covers a different name set than the fixture"
        );
