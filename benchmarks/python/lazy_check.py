...
CASES = [
    ("products.price", products, 2000, 13.0),  # 11.3µs measured x 1.15
    ('products[category = "Electronics"]', ecommerce, 1000, 27.6),  # 23.4µs measured x 1.15
    ("$sum(products[inStock].price)", ecommerce, 1000, 20.2),  # 17.1µs measured x 1.15
    (
        'products[price > 50 and inStock].{"name": name, "price": price, "vendor": vendor.name}',
        ecommerce,
        500,
        93.0,
    ),  # 79.0µs measured x 1.15 regression gate
]

failed = False
for src, data, iters, target in CASES:
    e = jsonatapy.compile(src)
    us = best_us(lambda e=e, data=data: e.evaluate(data), iters)
    ok = us <= target
    failed |= not ok
